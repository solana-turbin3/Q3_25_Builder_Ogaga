...
describe("anchor-vault", () => {
  ...
  it("Deposit 20 SOL", async () => {
    const tx = await program.methods
    .deposits(new anchor.BN(20 * anchor.web3.LAMPORTS_PER_SOL))
    .accountsPartial({
      user: provider.wallet.publicKey,
      vaultState,
      vault,
  ...
  it("Withdraw 10 SOL", async () => {
    const tx = await program.methods
    .withdrawls(new anchor.BN(10 * anchor.web3.LAMPORTS_PER_SOL))
    .accountsPartial({
      user: provider.wallet.publicKey,
      vaultState,
      vault,
