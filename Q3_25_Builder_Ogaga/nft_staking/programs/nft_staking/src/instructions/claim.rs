...
impl<'info> Claim<'info> {
    pub fn claim(&mut self, _bumps: &ClaimBumps) -> Result<()> {
        let points = self.user_account.points;
        let config = &self.config;
        
        // Calculate rewards based on points and points_per_stake
        let rewards_amount = (points as u64) * (config.points_per_stake as u64);
        ...
        if rewards_amount > 0 {
            ...
            mint_to(cpi_ctx, rewards_amount)?;
